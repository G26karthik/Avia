"""
Avia — GenAI Adapter (Vercel Serverless)
...
6. VEHICLE DETAILS:
   - Extract make, model, year, registration ONLY if explicitly present.
...
- "total_claim_amount": number or null — the dollar amount being claimed
- "incident_date": string in YYYY-MM-DD format or null
- "policy_start_date": string in YYYY-MM-DD format or null — only if explicitly stated
- "customer_tenure_months": number or null — calculated only if both dates are present
- "bodily_injuries": number or null — only if explicitly mentioned
...
- "policy_state": string or null — only if explicitly stated
- "vehicle_make": string or null — only if explicitly stated
- "vehicle_model": string or null — only if explicitly stated
- "vehicle_year": string or null — only if explicitly stated
- "vehicle_registration": string or null — only if explicitly stated
