...
# Seed demo claims if CSV exists
if os.path.exists(CSV_PATH):
    count = db.seed_claims_from_csv("org-demo-001", CSV_PATH, limit=50)
    if count > 0:
        print(f"Seeded {count} demo claims from {CSV_PATH}")
...
MAX_CLAIM_DATA_SIZE = 50000  # 50KB max for claim_data JSON
...
class DecisionRequest(BaseModel):
    action: str  # "escalate" | "genuine" | "defer"
    notes: Optional[str] = ""
...
def list_claims(current_user: dict = Depends(get_current_user)):
    ...
    # Return summary view (no full traces)
    summaries = []
    for c in claims:
...
# Required fields that must be extracted from the document
...
async def upload_and_create_claim(
    ...
    for safe_filename, content, ext in file_contents:
        ...
        # Multimodal GenAI insights per document
...
async def upload_document(claim_id: str, file: UploadFile = File(...),
    ...
    # Multimodal GenAI document insights (no OCR)
    cd = claim.get("claim_data", {}) or {}
...
def analyze_claim(claim_id: str, current_user: dict = Depends(get_current_user)):
    ...
    # For uploaded claims, require at least one document before analysis
    docs = db.get_claim_documents(claim_id)
    ...
    # 1. ML scoring (three buckets)
    ml_result = ml_engine.analyze_claim(cd)

    # 2. Gather document insights (skip for dataset claims with no docs)
...
@app.post("/api/seed")
def seed_claims():
    """Seed demo claims from CSV. Idempotent."""
